...
export function RequestPasswordResetForm() {
  ...
  return (
    <form action={action} className="space-y-4">
      ...
      <Button variant="dark" size="lg" fullWidth loading={pending} loadingLabel="Sending reset link…">
        Send reset link
      </Button>
    </form>
  );
...
export function UpdatePasswordForm() {
  ...
  return (
    <form action={action} className="space-y-4">
      ...
      />
      {state.error ? <InlineAlert tone="danger">{state.error}</InlineAlert> : null}
      <Button variant="primary" size="lg" fullWidth loading={pending} loadingLabel="Updating password…">
        Update password
      </Button>
    </form>
  );
